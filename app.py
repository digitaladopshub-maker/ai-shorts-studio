...
# App Header
st.title("🎬 Smart Pro AI Shorts Studio - Phase 3 (Audio Mixing)")
...
def detect_face_center(video_path, start_sec):
    try:
...
with st.sidebar:
    ...
    if option == "Paste URL (YouTube / FB / Insta)":
        ...
        if video_url and st.button("Fetch & Download Video"):
            with st.spinner("Downloading Video..."):
                if os.path.exists("/content/input_video.mp4"):
                    os.remove("/content/input_video.mp4")
                if os.path.exists("/content/preview_frame.jpg"):
                    os.remove("/content/preview_frame.jpg")
                
                dl_cmd = (
                    f'yt-dlp --no-check-certificates '
                    f'--extractor-args "youtube:player_client=ios,mweb" '
                    f'-f "b[ext=mp4]/best[ext=mp4]/best" '
                    f'-o "/content/input_video.mp4" "{video_url}"'
                )
        ...
        if uploaded_file is not None:
            if os.path.exists("/content/preview_frame.jpg"):
                os.remove("/content/preview_frame.jpg")
            with open("/content/input_video.mp4", "wb") as f:
    ...
    with tab_subs:
        ...
        if enable_subs:
            ...
            with col_preview:
                ...
                preview_frame_path = "/content/preview_frame.jpg"
                ...
                if enable_face_tracking:
                    f_x = detect_face_center("/content/input_video.mp4", target_time)
                    if f_x:
                        vf_crop = f"crop=ih*9/16:ih:clamp(x={f_x}-ih*9/32\,0\,in_w-ih*9/16):0,scale=540:960"

                subprocess.run(
                    f'ffmpeg -y -ss {target_time} -i "/content/input_video.mp4" -vframes 1 -vf "{vf_crop}" "{preview_frame_path}"', 
                    shell=True, capture_output=True
                )
                
                if os.path.exists(preview_frame_path):
                    img = Image.open(preview_frame_path)
    ...
    if st.button("🚀 Render Shorts Batch Now", type="primary", use_container_width=True):
        ...
        with st.spinner("Processing High-Quality Shorts with AI Engine..."):
            for clip_num, start_sec, duration_sec in tasks:
                cropped_file = f"/content/cropped_{clip_num}.mp4"
                final_file = f"/content/final_short_{clip_num}.mp4"
                
                vf_crop_hd = "crop=ih*9/16:ih,scale=1080:1920"
                if enable_face_tracking:
                    f_x = detect_face_center("/content/input_video.mp4", start_sec)
                    if f_x:
                        vf_crop_hd = f"crop=ih*9/16:ih:clamp(x={f_x}-ih*9/32\,0\,in_w-ih*9/16):0,scale=1080:1920"

                crop_cmd = (
                    f'ffmpeg -y -ss {start_sec} -i "/content/input_video.mp4" -t {duration_sec} '
                    f'-vf "{vf_crop_hd}" '
                    f'-c:v libx264 -preset ultrafast -crf 20 -c:a aac "{cropped_file}"'
                )
                subprocess.run(crop_cmd, shell=True)

                mixed_audio_file = f"/content/mixed_{clip_num}.mp4"
                if enable_bg_music and os.path.exists("/content/bg_music.mp3"):
                    mix_cmd = (
                        f'ffmpeg -y -i "{cropped_file}" -stream_loop -1 -i "/content/bg_music.mp3" '
                ...
                if enable_subs and model:
                    ...
                    ass_file = f"/content/subs_{clip_num}.ass"
                        ...
                        for segment in result['segments']:
                            if 'words' in segment:
                                ...
                                for i in range(0, len(words), words_per_line):
                                    ...
                                    if anim_effect == "Pop-In Scale (Fast Zoom)":
                                        anim_tag = r"{	(0,80,scx115scy115)	(80,160,scx100scy100)}"
                                    elif anim_effect == "Fade In / Fade Out":
                                        anim_tag = r"{ad(100,100)}"
                                    else:
                                        anim_tag = ""
